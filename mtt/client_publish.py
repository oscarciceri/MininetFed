...
class FederatedClient(object):
    def __init__(self, casa, x_train, y_train, x_test, y_test, model, target_scaler, features_scalers):
        self.casa = casa
    ...
    def publish_introduction(self, client):
        ...
        response = subscribe.simple(client_id, hostname=broker)
    ...
    def publish_agg(self,client):
        ...
        if self.chosen == 1:
            ...
            model_name = 'Modelos/best_model'+ self.client_paho_id  + '_' + str(self.round) + '.h5'
            checkpoint = ModelCheckpoint(model_name, monitor='val_loss', verbose=1, save_best_only=True, mode='min')
            history = self.model.fit(self.x_train, self.y_train, epochs=100, validation_split=0.10, batch_size=32,verbose=0,callbacks=[es,checkpoint])
    ...
    def connect_mqtt(self):
        ...
        client = mqtt_client.Client(self.client_paho_id)
    ...
    def run(self):
        ...
        while self.round < self.total_round :
            ...
            if self.round == self.total_round:
                ...
                self.rmse = mean_squared_error(y_test_rescaled, pred_rescaled, squared=False)
                self.score = r2_score(y_test_rescaled, pred_rescaled)
                model_path = './cluster_models/balanced_clusters/Original_client_publish/1_step_models/no_clustering.h5'
                if not os.path.isfile(model_path):
                    self.model.save(model_path)
                print(f'MAE:{self.mae}')
                print(f'RMSE:{self.rmse}')
                print(f'R2:{self.score}')
                with open('Resultados/balanced_clusters/Original_client_publish/GenomeDatasetClustered_1_step/no_clustering.csv','a') as fd:
