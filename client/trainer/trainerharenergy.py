...
class TrainerHarEnergy:
    ...
    def load_data(self):
        df = pd.read_csv(os.path.abspath(
            "client/data/pml.csv"), low_memory=False)
        ...
        if self.mode == "client":
            ...
            newDf = newDf[newDf[self.idColumn] == idslist[int(
                self.id % len(idslist))-1]].drop(columns=[self.idColumn])
            x_train, x_test, y_train, y_test = train_test_split(newDf.drop(
                columns=["classe"]).values, newDf["classe"].values, test_size=0.20, random_state=42)
            return x_train, y_train, x_test, y_test
        ...
        if self.mode == "random":
            ...
            chosen_cases = np.random.choice(
                total_cases, cases_per_client, replace=False)
        ...
        if self.mode == "client-disbalanced":
            ...
            # Remove the class that matches self.id
            newDf = newDf[newDf["classe"] != int((self.id-1) % n_classes)]
            x_train, x_test, y_train, y_test = train_test_split(newDf.drop(
                columns=["classe"]).values, newDf["classe"].values, test_size=0.20, random_state=42)
            return x_train, y_train, x_test, y_test
...
if __name__ == '__main__':
    trainer = TrainerHarEnergy(0, 'client')
